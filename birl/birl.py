...
class BIRL_BATCH():
    def __init__(self, expert_trace, grid_size, terminals, init, step_size=1.0, r_min=-10.0,
                 r_max=10.0, prior = 'uniform', birl_iteration = 2000, gamma = 0.9, prior_dict = {}, alpha=1.0, init_mdp = None):
    ...
    def run_birl(self):
        ...
        if self.init_mdp == None:
            mdp = self.create_rand_rewards() #pick a starting reward vector
        else:
            mdp = copy.deepcopy(self.init_mdp)
    ...
    def create_zero_rewards(self):
        return GridMDP([[0 for _ in range(self.n_columns)] for _ in range(self.n_rows)]
                       ...
                       r_min = self.r_min, r_max = self.r_max, gamma = self.gamma)
