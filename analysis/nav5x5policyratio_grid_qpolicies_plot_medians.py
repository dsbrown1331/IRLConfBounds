    ...
    for num_mcmc_samples in mcmc_samples:
        for bound_type in bounds:
            ...
            for loss in loss_ranges[num_demos]:
                ...
                median_bounds.append(np.nanmedian(predicted))
                true_perf_ratio.append(actual)
                print "true bound", actual
                print "predicted bounds", predicted
                percentile_upper_bounds.append(bound_methods.percentile(predicted,0.975) - np.nanmedian(predicted))
                percentile_lower_bounds.append(np.nanmedian(predicted) - bound_methods.percentile(predicted, 0.025))
                print 
            ...
            plt.errorbar(true_perf_ratio, median_bounds, yerr=[percentile_lower_bounds, percentile_upper_bounds], fmt=fmts[bounds.index(bound_type)], label=bound_type, lw=1)
